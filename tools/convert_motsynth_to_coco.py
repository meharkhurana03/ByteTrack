...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        annos_path = os.path.join(DATA_PATH, 'mot_annotations')
        imgs_path = os.path.join(DATA_PATH, 'frames')
        out_path = os.path.join(OUT_PATH, '{}.json'.format(split))
        out = {'images': [], 'annotations': [], 'videos': [],
               'categories': [{'id': 1, 'name': 'pedestrian'}]}
        seqs = os.listdir(annos_path)
        image_cnt = 0
        ann_cnt = 0
        video_cnt = 0
        tid_curr = 0
        ...
        for seq in sorted(seqs):
            ...
            if split != 'test':
                ...
                anns = np.loadtxt(seq_anno_path, dtype=np.float32, delimiter=',')
